...
use non_zero_affine::NonZeroNonNativeAffineVar;
...
#[must_use]
pub struct NonNativeProjectiveVar<
...
> {
    ...
    /// The z-coordinate.
    pub z: NonNativeFieldVar<SimulationF, ConstraintF>,
...
impl<P, ConstraintF, SimulationF> NonNativeAffineVar<P, ConstraintF, SimulationF>
where
    P: SWModelParameters<BaseField = SimulationF>,
    ConstraintF: PrimeField,
    SimulationF: PrimeField,
...
impl<P, ConstraintF, SimulationF> ToConstraintFieldGadget<ConstraintF>
    for NonNativeAffineVar<P, ConstraintF, SimulationF>
where
    P: SWModelParameters<BaseField = SimulationF>,
    ConstraintF: PrimeField,
    SimulationF: PrimeField,
...
impl<P, ConstraintF, SimulationF> R1CSVar<ConstraintF>
    for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
    P: SWModelParameters<BaseField = SimulationF>,
    ConstraintF: PrimeField,
    SimulationF: PrimeField,
...
impl<P, ConstraintF, SimulationF> NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
    P: SWModelParameters<BaseField = SimulationF>,
    ConstraintF: PrimeField,
    SimulationF: PrimeField,
{
    ...
    pub fn new(
        x: NonNativeFieldVar<SimulationF, ConstraintF>,
        y: NonNativeFieldVar<SimulationF, ConstraintF>,
        z: NonNativeFieldVar<SimulationF, ConstraintF>,
    ) -> Self {
        Self {
            x,
            y,
    ...
    pub fn to_affine(&self) -> Result<NonNativeAffineVar<P, ConstraintF, SimulationF>, SynthesisError> {
        if self.is_constant() {
            let point = self.value()?.into_affine();
            let x = NonNativeFieldVar::<SimulationF, ConstraintF>::new_constant(ConstraintSystemRef::None, point.x)?;
            let y = NonNativeFieldVar::<SimulationF, ConstraintF>::new_constant(ConstraintSystemRef::None, point.y)?;
            let infinity = Boolean::constant(point.infinity);
            Ok(NonNativeAffineVar::new(x, y, infinity))
        } else {
            let cs = self.cs();
            let infinity = self.is_zero()?;
            let zero_x = NonNativeFieldVar::<SimulationF, ConstraintF>::zero();
            let zero_y = NonNativeFieldVar::<SimulationF, ConstraintF>::one();
            // Allocate a variable whose value is either `self.z.inverse()` if the inverse
            // exists, and is zero otherwise.
            let z_inv = NonNativeFieldVar::<SimulationF, ConstraintF>::new_witness(ark_relations::ns!(cs, "z_inverse"), || {
            ...
            z_inv.mul_equals(&self.z, &NonNativeFieldVar::<SimulationF, ConstraintF>::from(infinity.not()))?;
    ...
    pub fn new_variable_omit_on_curve_check(
        ...
        let x = NonNativeFieldVar::<SimulationF, ConstraintF>::new_variable(ark_relations::ns!(cs, "x"), || x, mode)?;
        let y = NonNativeFieldVar::<SimulationF, ConstraintF>::new_variable(ark_relations::ns!(cs, "y"), || y, mode)?;
        let z = NonNativeFieldVar::<SimulationF, ConstraintF>::new_variable(ark_relations::ns!(cs, "z"), || z, mode)?;
    ...
    pub(crate) fn add_mixed(
        &self,
        other: &NonZeroNonNativeAffineVar<P, ConstraintF, SimulationF>,
    ) -> Result<Self, SynthesisError> {
        ...
        let axz = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&xz_pairs); // 12
        ...
        let azz = mul_by_coeff_a::<P, ConstraintF, SimulationF>(z1); // 20
        let xx3_p_azz = xx.double().unwrap() + &xx + &azz; // 18, 19, 22

        let bxz3 = &xz_pairs * three_b; // 21
        let b3_xz_pairs = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&(&xx - &azz)) + &bxz3; // 23, 24, 25
        ...
        Ok(NonNativeProjectiveVar::new(x, y, z))
    }
    ...
    fn fixed_scalar_mul_le(
        &self,
        mul_result: &mut Self,
        multiple_of_power_of_two: &mut NonZeroNonNativeAffineVar<P, ConstraintF, SimulationF>,
        ...
        // `accumulator`, because `NonZeroNonNativeAffineVar::add_unchecked` doesn't
...
impl<P, ConstraintF, SimulationF> CurveVar<SWProjective<P>, ConstraintF> for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
    where
        P: SWModelParameters<BaseField = SimulationF>,
        ConstraintF: PrimeField,
        SimulationF: PrimeField,
{
    ...
    fn zero() -> Self {
        Self::new(NonNativeFieldVar::zero(), NonNativeFieldVar::one(), NonNativeFieldVar::zero())
    }
    ...
    fn double_in_place(&mut self) -> Result<(), SynthesisError> {
        ...
        let axz2 = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&xz2); // 8
        ...
        let azz = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&zz); // 16
        let b3_xz_pairs = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&(&xx - &azz)) + &bxz3; // 15, 16, 17, 18, 19
    ...
    fn scalar_mul_le<'a>(
        ...
        let non_zero_self = NonZeroNonNativeAffineVar::new(x, y);
...
impl<P, ConstraintF, SimulationF> ToConstraintFieldGadget<ConstraintF> for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
    where
        P: SWModelParameters<BaseField = SimulationF>,
        ConstraintF: PrimeField,
        SimulationF: PrimeField,
...
fn mul_by_coeff_a<
    ...
    SimulationF: PrimeField,
>(
    f: &NonNativeFieldVar<SimulationF, ConstraintF>,
) -> NonNativeFieldVar<SimulationF, ConstraintF>
{
    if !P::COEFF_A.is_zero() {
        f * P::COEFF_A
    } else {
        NonNativeFieldVar::<SimulationF, ConstraintF>::zero()
    }
}
...
impl_bounded_ops!(
    ...
    AddAssign,
    add_assign,
    |mut this: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>, mut other: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>| {
        ...
        if other.is_constant() {
            ...
            if other.is_zero() {
                ...
                let x = NonNativeFieldVar::constant(other.x);
                let y = NonNativeFieldVar::constant(other.y);
                this.add_mixed(&NonZeroNonNativeAffineVar::new(x, y)).unwrap()
            }
        } else {
            ...
            let axz = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&xz_pairs); // 19
            ...
            let azz = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&zz);
            let xx3_p_azz = xx.double().unwrap() + &xx + &azz; // 25, 26, 27, 29

            let bxz3 = &xz_pairs * three_b; // 28
            let b3_xz_pairs = mul_by_coeff_a::<P, ConstraintF, SimulationF>(&(&xx - &azz)) + &bxz3; // 30, 31, 32
            ...
            NonNativeProjectiveVar::new(x, y, z)
        }

    },
    |this: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>, other: SWProjective<P>| {
        this + NonNativeProjectiveVar::constant(other)
    },
    (P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField),
...
impl_bounded_ops!(
    ...
    SubAssign,
    sub_assign,
    |this: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>, other: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>| this + other.negate().unwrap(),
    |this: &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>, other: SWProjective<P>| this - NonNativeProjectiveVar::constant(other),
    (P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField),
);

impl<'a, P, ConstraintF, SimulationF> GroupOpsBounds<'a, SWProjective<P>, NonNativeProjectiveVar<P, ConstraintF, SimulationF>>
    for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField 
{
...
impl<'a, P, ConstraintF, SimulationF> GroupOpsBounds<'a, SWProjective<P>, NonNativeProjectiveVar<P, ConstraintF, SimulationF>>
    for &'a NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField 
{
}
...
impl<P, ConstraintF, SimulationF> EqGadget<ConstraintF> for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField,             
{
    fn is_eq(
...
impl<P, ConstraintF, SimulationF> AllocVar<SWAffine<P>, ConstraintF>
    for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField,        
{
...
impl<P, ConstraintF, SimulationF> AllocVar<SWProjective<P>, ConstraintF>
    for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField,        
{
...
impl<P, ConstraintF, SimulationF> ToBitsGadget<ConstraintF> for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField,     
{
    fn to_bits_le(
...
impl<P, ConstraintF, SimulationF> ToBytesGadget<ConstraintF> for NonNativeProjectiveVar<P, ConstraintF, SimulationF>
where
P: SWModelParameters<BaseField = SimulationF>, ConstraintF: PrimeField, SimulationF: PrimeField,     
{
    fn to_bytes(
